...
const buildChoke = () => { //fixed length, no payload. 
    //Choke: <len=0001><id=0>
...
const buildUnchoke = () => { //fixed length, no payload. 
...
const buildInterested = () => {//fixed length, no payload. 
...
const buildUninterested = () => {//fixed length, no payload. 
...
const buildHave = (payload) => { //fixed length, payload = 0based index of recently downloaded piece and verified using hash.
...
const buildBitfield = (bitfield) => {// Variable length. Sent immediately after hanshake. Optional. 
...
const buildRequest = (payload) => {// fixed length. Used to request a block. Contents of payload: index, begin, length
...
const buildPiece = (payload) => {// Variable length. Sent after a request. Contents of payload: index, begin, block
...
const buildCancel = (payload) => {// fixed length. Used to cancel a block request. Contents of payload: index, begin, length
...
const buildPort = (payload) => {// fixed length. Sent after handshake to announce the port number.
